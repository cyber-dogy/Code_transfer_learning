...
#%%这一段就是数据预处理了。首先修改文件路径后，把数据取进来

path_norm = "D:\signal_deal\signal\西储大学轴承数据中心网站\\Normal Baseline Data\\97.mat"#第一类数据来自97号
file_norm = scio.loadmat(path_norm)
data_norm = file_norm['X097_DE_time']
data_norm = np.array(data_norm)
path_fault = "D:\signal_deal\signal\西储大学轴承数据中心网站\\12k Drive End Bearing Fault Data\\105.mat"#第二类数据来自105号
...
data_1s = np.empty((0, 100, 9))#这就是数据的规模size（batch，high，weight），以下同理
data_2s = np.empty((0, 100, 9))#因为我用了个2D——CNN，所以你得把这个数据想象成一个图片，100行，9列，每列是一个来自轴承数据的数据段。
for z in range(76):
...
#%%输入网络的数据包，是一个dataset子类，需要用dataloader加载（不懂去参考pytorch文档或者CSDN百度都行）
